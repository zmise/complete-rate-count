...
+function ($) {
  ...
  Tree.prototype.render = function (data, searchValue) {
    ...
    if (typeof data === 'string') {
      ...
      if (this.option.multiple === false) this.el.find('.panel-checkbox > input').hide()
...
+ function($) {
  ...
  Select.prototype.render = function() {
    ...
    if (!option.search && !option.multiple && !option.treeOption) {
      $ctrl.addClass('scaret'); // before('<span class="scaret"></span>')
    }
  ...
  MMPaginator.prototype = {
    ...
    , params: function(){
      ...
      if (opts.params) {
        ...
        params[opts.limitParamName] = $limitList.val() || $el.data('limit') || opts.limit;
      }
      return params;
    }
...
+function ($) {
  var MMGrid = function (element, options) {
    ...
    this._loadCount = 0;
    this._ajax = null; //保留 ajax 对象供外部 promise 控制数据加载
    this.opts = options;
  ...
  MMGrid.prototype = {
    ...
    _initLayout: function ($el) {
      ...
      var mmGrid = [
        ...
        '<a class="mmg-btnBackboardUp">关闭</a>',
        '</div>',
        '<div class="mmg-bodyWrapper"></div>',
        '<a class="mmg-btnBackboardDn">展开</a>',
    ...
    _initOptions: function () {
      ...
      if (cols) {
        ...
        for (var colIndex = 0; colIndex < cols.length; colIndex++) {
          ...
          fchtml += '/><span>';
          if (col.titleHtml) {
            dchtml += col.titleHtml;
            fchtml += col.titleHtml;
          } else if (col.title) {
            dchtml += col.title;
            fchtml += col.title;
          } else {
    ...
    _fullWidthRows: function () {
      ...
      for (var i = 0; i < leafCol.length; i++) {
        var col = leafCol[i];
        var $th = $ths.eq(i);
        if ($th.is(':visible') && col.hidden !== true) {
          thsArr.push($th);
        }
      }

      var increaseWidth = fitWidth / thsArr.length;
        ...
        try {
          ...
          */
          ...
          if (data && $.isArray(data[opts.root])) {
            data = $.extend(false, args, data);
          }
          that.$body.triggerHandler('loadSuccess', data, result);
        } catch (e) {
          that._hideLoading();
          that._showLoadError();
          throw e;
    ...
    load: function (args) {
      try {
        ...
        if ($.isArray(args)) {
          ...
          this._ajax = this._loadAjax(args);
        } else if (opts.items) {
    ...
    _buildFrozenColumn: function(col, data) {
      ...
      for (var i = 0; i < data.length; i++) {
        html += '<tr class="row-' + i + (data[i].isSelected ? ' selected' : '') + '"><td class="col-'
          + col.index + (col.nowrap ? ' nowrap' : '') + '"' + (col.nowrap ? ' title="' + data[i].text + '"' : '') + '>'
        if (col.checkCol) {
        ...
        if (col.frozen) {
          ...
          if ($frozen && $frozen.length > 0) {
            ...
            $frozen = $(
              ...
              '          data-index="' + i + '">' + (col.titleHtml || col.title) + '<i class="glyphicon"></i></th>' +
              '      </tr>' +
              '    </thead>' +
              '  </table>' +
    ...
    frozenCols: function(idxs, value) {
      var $backboard = this.$backboard
      idxs = $.isArray(idxs) ? idxs : [idxs]
      var cols = this._leafCols()
      $.each(idxs, function(i, item) {
        var result = (value !== false)
        cols[item].frozen = result
        cols[item].hidden = result
        $backboard.find('.frozen-cols :checkbox:eq(' + item + ')').prop('checked', result)
      })
      this._setColsWidth()
      this._frozenColumns()
    },

    hiddenCols: function(idxs, value) {
      var $backboard = this.$backboard
      idxs = $.isArray(idxs) ? idxs : [idxs]
      var cols = this._leafCols()
      $.each(idxs, function(i, item) {
        ...
        cols[item].hidden = result
        $backboard.find('.hidden-cols :checkbox:eq(' + item + ')').prop('checked', !result)
        $backboard.find('.frozen-cols :checkbox:eq(' + item + ')').prop('disabled', result)
        cols[item].frozen = false
      })
      this._setColsWidth()
      this._frozenColumns()
    }
...
(function (window, $, undefined) {
    ...
    datepicker.prototype = {
        ...
        _handleBubbled: function() {
            if (!this.opts.bubbled) {
                this.$datepicker.on('click.bs.datepicker', function (e) {
                    e.stopPropagation();
                });
            }
        }
